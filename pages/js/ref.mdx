...
<Boxes id="eqeqeq">
  <Box>
    **В отличие от `==` и `!=`, операторы `===` и `!==` не приводят разные типы друг к другу.**
    ...
    <small>Подробные правила сравнения, которыми пользуется JavaScript, описаны в [The Abstract Equality Comparison Algorithm](https://262.ecma-international.org/5.1/#sec-11.9.3).</small>
  </Box>
  <Box>
...
<Boxes id="cannot-declare-a-let-variable-twice">
  <Box>
