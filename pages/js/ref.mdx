...
<Boxes>
  ...
  <Box>
    <CodeAnn ok="message = 'Hello, world!'">
      ```js
...
<Boxes>
  <Box>
    Если один из операндов — строка, то остальные будут автоматически преобразованы в строку:
  </Box>
  <Box>
    <CodeAnn ok="message = 'Прошло 7 дней'">
      ```js
      let days = 7;
      let message = 'Прошло ' + days + ' дней';
      ```
    </CodeAnn>
  </Box>
</Boxes>
...
<Boxes>
  <Box>
    Также можно написать `-` перед выражением, чтобы поменять его знак:
  </Box>
  <Box>
    <CodeAnn ok="Работает">
      ```js
      let a = 3;
      console.log(-a);     // Выводит -3
      console.log(-(-a));  // Выводит 3
      ```
    </CodeAnn>
  </Box>
...
<Boxes>
  <Box>
    **Нельзя использовать `-`, чтобы отрезать от строки кусочек.**
    
    Оператор `-` просто попытается преобразовать обе строки в числа, не сможет этого сделать и вернет `NaN` ("not a number").
    
    Чтобы отрезать кусочек строки, используйте метод `.slice`.
  </Box>
  <Box>
    <CodeAnn warn="В string будет NaN, а не 'a'">
      ```js
      let string = 'ab' - 'b';  // NaN
      ```
    </CodeAnn>
...
<Boxes>
  <Box>
    Чтобы поделить нацело, можно округлить число после деления с помощью `Math.floor`.

    <p className="small">Чтобы узнать, делится ли одно число нацело на другое, лучше использовать оператор `%`. Прочитайте про него в соответствующем разделе.</p>
  </Box>
  <Box>
    <CodeAnn ok="a = 2.5, b = 2">
      ```js
      let a = 10 / 4;                // 2.5
      let b = Math.floor(10 / 4);    // 2
      ```
    </CodeAnn>
  </Box>
</Boxes>
...
<Boxes>
  <Box>
    Деление на ноль возвращает `Infinity` или `-Infinity`. Деление `0 / 0` возвращает `NaN`.
  </Box>
  <Box>
    <CodeAnn ok="Так можно">
      ```js
      let a = 1 / 0;     // Infinity
      let b = -1 / 0;    // -Infinity
      let c = 0 / 0;     // NaN
...
<Boxes>
  <Box>
    Оператор `%` часто используется для проверки, делится ли одно число нацело на другое. Если делится, остаток будет 0.
  </Box>
  <Box>
    <CodeAnn ok="Напечатает 'Делится'">
      ```js
      let a = 100;
      // mark[5:15]
      if (a % 2 === 0) {
...
<Boxes>
  <Box>
    Еще `%` можно использовать, чтобы сбрасывать чиcло до нуля, когда оно дошло до какого-то предела:
  </Box>
  <Box>
    <CodeAnn ok="Напечатает 0, 1 .. 99 и так десять раз">
      ...
      for (let i = 0; i < 1000; i++) {
        console.log(n);
        // mark[3:17]
        n = (n+1) % 100;  // Теперь n ограничено между 0 и 99
      }
      ```
    </CodeAnn>
  </Box>
...
<B.Alert variant="info">
  Операторы `+=`, `-=`, `*=`, `/=` — это сокращенные операции присваивания.

  * Сложение: `a += n` значит `a = a + n`
  * Вычитание: `a -= n` значит `a = a - n`
  * Умножение: `a *= n` значит `a = a * n`
  * Деление: `a /= n` значит `a = a / n`
</B.Alert>

<Boxes>
  <Box>
    Например, `a += 10` увеличит переменную `a` на 10.
    
    Соответственно, `b -= 10` уменьшит переменную `b` на 10.
  </Box>
  <Box>
    <CodeAnn ok="Теперь a = 110, b = 90">
      ```js
      let a = 110, b = 90;
      a += 10;
      b -= 10;
...
<Boxes>
  <Box>
    Операторы `*=` и `/=` тоже существуют, но используются реже. Они умножают и делят переменную соответственно.

    <small>`%=` и `**=` тоже есть, но они вообще редкие.</small>
  </Box>
  <Box>
...
<Boxes>
  <Box>
    Еще можно раскидать массив или объект по переменным — подробнее об этом читайте на learn.javascript.ru в статье [Деструктурирующее присваивание](https://learn.javascript.ru/destructuring-assignment).
  </Box>
  <Box>
    <CodeAnn ok="x = 'hello', y = 'world'">
      ```js
      // mark
...
<Boxes id="identifier-has-already-been-declared">
  <Box>
    Если переменная, которую пытается создать `let`, уже есть, то будет ошибка:
  </Box>
  <Box>
