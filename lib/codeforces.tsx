...
const task110A: CodeforcesTaskData = {
  code: '110A',
  title: 'Почти счастливое число',
  link: 'https://codeforces.com/problemset/problem/110/A',
  tags: ['Строки'],
  statement: markdown`
    Дано число $N$ (от $1$ до $10^{18}$).
    
    Надо посчитать, сколько в нем цифр $4$ и $7$, и вывести \`YES\`, если _это количество_ само по себе состоит только из цифр $4$ и $7$.

    Например:
      * $N = 47$ — количество $2$ — ответ \`NO\`;
      * $N = 7747$ — количество $4$ — ответ \`YES\`;
      * $N = 707437$ — количество $4$ — ответ \`YES\`;
      * $N = 47474747$ — количество $8$ — ответ \`NO\`;
      * $N = \\underbrace{44444 \\ldots 44}_{\\text{47 четверок}}$ — количество $47$ — ответ \`YES\` (однако заметьте, что такого большого числа вам не могут дать).
  `,
  hints: [
    markdown`
      Так как число может не влезать в стандартный тип \`number\`, его лучше считывать как строку — то есть, не \`Number(readline())\`, а просто \`readline()\`.
    `,
    markdown`
      Чтобы посчитать, сколько в числе цифр $4$ и $7$, можно
        * пройтись по нему циклом,
        * или воспользоваться регулярными выражениями,
        * или перевести его в массив и посчитать цифры $4$ и $7$ с помощью метода \`.filter\`.
    `,
    markdown`
      Так как число не больше $10^{18}$, в нем будет максимум $19$ цифр.
      
      То есть, единственные "хорошие" варианты количества цифр $4$ и $7$ в нем — это, собственно, $4$ и $7$.
    `,
  ],
}
...
export const codeforcesTasks: CodeforcesTaskData[] = [
  ...
  task282A, // Bit++
  task110A, // Почти счастливое число
